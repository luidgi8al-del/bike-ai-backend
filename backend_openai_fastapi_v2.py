...
9bis. Si utilisateur donne une duree explicite dans son message ou si temps_disponible_min est renseigne, cette duree est une limite stricte : duree_totale_min et la somme de workout_steps doivent correspondre exactement a cette duree, sans ajouter de minutes bonus.
...
83bis. Si utilisateur demande 65 minutes, ne propose jamais 70, 75 ou 80 minutes : ajuste les blocs, les recuperations ou les parties endurance pour totaliser 65 minutes.
...
Règles :
1. SynthÃƒÆ’Ã‚Â©tise TOUTES les donnÃƒÆ’Ã‚Â©es disponibles ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ne te limite pas au TSB.
...
def normalize_workout_steps(
    ...
    normalized_steps = fit_steps_to_requested_duration(normalized_steps, requested_minutes)
    data["workout_steps"] = normalized_steps
    total_seconds = sum(int(step.get("duration_sec") or 0) for step in normalized_steps)
    if total_seconds > 0:
        data["duree_totale_min"] = int(requested_minutes) if is_valid_requested_minutes(requested_minutes) else max(1, round(total_seconds / 60))
    return data


def is_valid_requested_minutes(requested_minutes: Optional[int]) -> bool:
    ...
    try:
        ...
        return False
    return 10 <= minutes <= 300


def fit_steps_to_requested_duration(
    steps: List[Dict[str, Any]],
    requested_minutes: Optional[int],
) -> List[Dict[str, Any]]:
    """
    Dernier filet de securite : la duree demandee par l'utilisateur est prioritaire.
    On garde les efforts qualitatifs, puis on ajuste les transitions/endurance/recuperations.
    """
    if not steps or not is_valid_requested_minutes(requested_minutes):
        return steps

    target_seconds = int(requested_minutes) * 60
    fitted = [dict(step) for step in steps]
    total_seconds = sum(int(step.get("duration_sec") or 0) for step in fitted)
    delta = total_seconds - target_seconds

    if abs(delta) <= 15:
        return fitted
    if delta > 0:
        return trim_steps_duration(fitted, delta)
    return add_easy_time_before_cooldown(fitted, -delta)


def trim_steps_duration(steps: List[Dict[str, Any]], seconds_to_remove: int) -> List[Dict[str, Any]]:
    remaining = int(seconds_to_remove)
    result = [dict(step) for step in steps]

    def step_min_duration(step: Dict[str, Any]) -> int:
        duration = int(step.get("duration_sec") or 0)
        intensity = str(step.get("intensity") or "active")
        text = step_search_text(step)
        if is_hard_step(step):
            return duration
        ...
        if any(word in text for word in EASY_WORDS + ["endurance", "transition", "ajustement"]):
            return 0
        return min(duration, 60)

    def trim_pass(predicate) -> None:
        ...
        for index in range(len(result) - 1, -1, -1):
            ...
            if not predicate(step):
                continue
            duration = int(step.get("duration_sec") or 0)
            min_duration = step_min_duration(step)
            removable = max(0, duration - min_duration)
            if removable <= 0:
                continue
            removed = min(removable, remaining)
            new_duration = duration - removed
            remaining -= removed
            if new_duration <= 0:
                result.pop(index)
            else:
                step["duration_sec"] = new_duration

    trim_pass(lambda step: not is_hard_step(step) and any(word in step_search_text(step) for word in ["endurance facile", "transition", "ajustement", "facile avant", "finir par"]))
    trim_pass(lambda step: not is_hard_step(step) and str(step.get("intensity") or "") == "recovery")
    trim_pass(lambda step: not is_hard_step(step) and str(step.get("intensity") or "") == "active" and any(word in step_search_text(step) for word in EASY_WORDS + ["endurance"]))
    trim_pass(lambda step: not is_hard_step(step) and str(step.get("intensity") or "") in {"warmup", "cooldown"})
    ...
    if remaining > 0:
        for index in range(len(result) - 1, -1, -1):
            ...
            if duration <= 1:
                continue
            min_duration = 30 if duration > 30 else 1
            removable = max(0, duration - min_duration)
            removed = min(removable, remaining)
            if removed <= 0:
                continue
            step["duration_sec"] = duration - removed
            remaining -= removed

    return result


def add_easy_time_before_cooldown(steps: List[Dict[str, Any]], seconds_to_add: int) -> List[Dict[str, Any]]:
    if seconds_to_add <= 0:
        return steps
    filler = {
        "name": "Endurance facile ajustement",
        "duration_sec": int(seconds_to_add),
        "target_type": "ftp_percent",
        "target_low": 45.0,
        "target_high": 55.0,
        "intensity": "recovery",
        "shape": "steady",
        "notes": "Temps ajoute pour respecter la duree demandee sans ajouter d'intensite."
    }
    result = [dict(step) for step in steps]
    for index, step in enumerate(result):
        if step.get("intensity") == "cooldown":
            result.insert(index, filler)
            return result
...
def openai_recommendation(req: CoachRequest) -> CoachResponse:
    ...
    user_payload = {
        ...
        "contrainte_duree_stricte_min": req.day_data.temps_disponible_min,
    ...
    data = normalize_workout_steps(data, req.profile, req.day_data.temps_disponible_min)
